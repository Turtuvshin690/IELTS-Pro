...
const DESC = 'Part 1 intro · Part 2 cue card · Part 3 discussion';
  ...
  return (
    <div className="mx-auto max-w-5xl p-6" data-testid="speaking-list">
      <div className="flex items-center gap-2.5">
        ...
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight">Speaking</h1>
          <p className="mt-0.5 text-sm text-zinc-500">11–14 min · Parts 1–3 · mic required · <span className="font-semibold text-emerald-700">Free</span></p>
        </div>
      </div>
      <input
        value={q}
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        {filtered.map((t) => (
          <div key={t.id} className="flex flex-col rounded-xl border border-zinc-200 bg-white p-5 shadow-card" data-testid={`speaking-card-${t.id}`}>
            <div className="flex items-center justify-between"><span className="text-xl">🎤</span><span className="rounded bg-ink px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white">11-14 min</span></div>
            ...
            <Link to={`/speaking/${t.id}`} className="mt-4 rounded-lg bg-emerald-600 px-4 py-2 text-center text-sm font-bold text-white hover:bg-emerald-700" data-testid={`start-speaking-${t.id}`}>Practice free →</Link>
          </div>
        ))}
      </div>
      {filtered.length === 0 && <div className="mt-6 rounded-xl border border-dashed bg-white p-8 text-center text-sm text-zinc-500" data-testid="speaking-empty">{tests.length === 0 ? 'No speaking tests. Import or restart to seed.' : 'No tests match your search.'}</div>}
    </div>
  );
}
